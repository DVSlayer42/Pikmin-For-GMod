...
*****************************************]]
function ENT:Mdl()
	if (IsValid(self.PikMdl)) then return self.PikMdl end
...
function ENT:GetAnim()
	local mdl = self:Mdl()
	if (mdl.Anim) then return tostring(mdl.Anim) end
...
function ENT:GetPikLevel()
	if (self.PikLevel) then return tonumber(self.PikLevel) end

	return nil
end

function ENT:GetPikType()
	if (self.PikClr) then return tostring(self.PikClr) end
...
function ENT:CreatePikRagdoll(dis)
	...
	for i = 1, rag:GetPhysicsObjectCount() do
		...
		if (IsValid(bone)) then
			...
			end --is this for the dissolve effect?
		end
	end
...
local function PikminCreate(ply, cmd, args)
	...
	if (args[1] == "red" or args[1] == "yellow" or args[1] == "blue" or args[1] == "purple" or args[1] == "white" or args[1] == "random") then
		...
		if (clr ~= "purple" and clr ~= "white") then
...
function ENT:Disband()
	if (not self.Attacking and not IsValid(self.AtkTarget)) then
	...
	end --Don't disband if we're attacking or charging after an enemy, our leader still wants the kill!
end
...
function ENT:LatchOn(ent)
	if (IsValid(ent) and (ent:IsNPC() or ent:IsPlayer()) and (self.JustThrown or IsValid(self.AtkTarget))) then
		if (self:GetPikType() == "purple" and self.JustThrown) then
			ent:TakeDamage(10)
...
function ENT:Die()
	...
	timer.Simple(math.Rand(1.6, 2.5), function()
		if (IsValid(rag)) then
	...
	end) --Give it some random-ness, so they don't die in order so much
end
...
function ENT:OnRemove()
	...
	if (self.Carrying and IsValid(self.CarryEnt)) then
...
function ENT:OnTakeDamage(dmg)
	...
	if (self.PikHP <= 0) then
		if (self:GetPikType() == "white") then
			...
			timer.Create("PikPoisonfor " .. self:EntIndex(), .9, math.random(10, 12), function(pos, o)
				...
				for k, v in pairs(ents.FindInSphere(poisonpos, 125)) do
					if (v:IsPlayer() and v ~= o) then
						...
						v:TakeDamage(math.random(1, 2), (o or self), self)
					end
		...
		end --Poison the bastards that killed us
		...
		if (self:GetPikLevel() ~= 1) then
			...
			if (self:GetPikType() == "white" or self:GetPikType() == "purple") then
				r = 220
				g = 100
				b = 150
			end
...
function ENT:Think()
	...
	if (vel:Length() <= 5 and not self.Dismissed and not self.Victim and (not self.ShouldSpin or not self.JustThrown) and not self.RejoinAnim and not self.Sucking) then
		self:SetAnim("idle")
	end

	if (vel:Length() >= 6 and not self.Dismissed and not self.Victim and (not self.ShouldSpin or not self.JustThrown) and not self.RejoinAnim and not self.Sucking) then
		self:SetAnim("running")
	end

	--Fail-safe
	if ((not IsValid(self.Olimar) or not self.Olimar:Alive()) and not self.Dismissed) then
		self:Disband()
	end
	...
	if (not self.Attacking and CurTime() >= self.LastPullApart) then
		self.LastPullApart = (CurTime() + .75)

		for k, v in pairs(ents.FindByClass("pikmin")) do
			if (v ~= self and v.Olimar == self.Olimar and pos:Distance(v:GetPos()) <= 10) then
				local phys = self:GetPhysicsObject()
	...
	if (IsValid(self.Olimar)) then
		if (self.CanMove) then
			...
			if (CurTime() >= self.LastHop) then
				...
				if (self:IsOnFire() and self:GetPikType() ~= "red") then
	...
	if (self:WaterLevel() >= 1) then
		...
		if (self:GetPikType() == "blue") then
			...
			if (not self.DrowningTimeBool) then
		...
		if (not self.PlayDrownSound) then
		...
		if (IsValid(self.Victim)) then
			if (CurTime() >= self.NextAttack) then
				...
				if ((self.Victim:Health() - dmg) <= 0) then
					for k, v in pairs(ents.FindByClass("pikmin")) do
						if (v.Victim == self.Victim and v ~= self) then
...
function ENT:PhysicsCollide(data, phys)
	if (self.JustThrown and data.HitEntity:IsWorld()) then
...
function ENT:StartTouch(thing)
	if (not self.Attacking) then
		...
		elseif (thing:IsNPC() and thing:GetClass() ~= "npc_rollermine") then
			self:LatchOn(thing)
		end

		if (thing:GetClass() == "prop_combine_ball") then
			if (IsValid(self) and not self.Dissolving) then
				self:EmitSound("pikmin/pikmin_die.wav", 100, math.random(95, 110))
