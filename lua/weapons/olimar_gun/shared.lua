...
function SWEP:Deploy()
	...
	timer.Create("PikDeployIdleAnim" .. self.Owner:UniqueID(), 1.2, 1, function()
...
function SWEP:Think()
	if not self.Whistling then return end
...
local function PikSWepKeyPress(ply, key)
	if (SERVER) then
		if (IsValid(ply) and ply:Alive()) then
			if (IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "olimar_gun") then
				if (key == IN_ATTACK) then
					local piks = {}
					local check = false

					for k, v in pairs(ents.FindByClass("pikmin")) do
						if (v.Olimar == ply and not v.Attacking) then
							local pos = ply:GetPos()
							local ppos = v:GetPos()

							if (ppos:Distance(pos) <= 200 and not v.JustThrown) then
								check = true
								table.insert(piks, v)
							end
						end
					...
					if (check) then
						...
						timer.Create("PikPrimaryGrabSound" .. ply:UniqueID(), .8, 1, function(ply)
				...
				if (key == IN_RELOAD) then
					if (not ply:KeyDown(IN_USE)) then
						...
						if (tr.Hit) then
							...
							timer.Create("PikReloadIdleAnim" .. ply:UniqueID(), 1.45, 1, function()
								ply:GetActiveWeapon():SendWeaponAnim(ACT_VM_IDLE)
							end)

							timer.Create("PikWhistlefor" .. ply:UniqueID(), 1.4, 1, function()
						...
						if (doanim) then
							...
							timer.Create("PikReloadIdleAnim" .. ply:UniqueID(), .75, 1, function()
								ply:GetActiveWeapon():SendWeaponAnim(ACT_VM_IDLE)
							end)
...
local function PikSWepKeyRelease(ply, key)
	if (SERVER) then
		if (IsValid(ply) and ply:Alive()) then
			if (IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "olimar_gun") then
				if (key == IN_ATTACK) then
					if (IsValid(throwpikmin) and ply.HasPikmin) then
						timer.Destroy("PikPrimaryIdleAnim" .. ply:UniqueID())
						timer.Destroy("PikDeployIdleAnim" .. ply:UniqueID())
						...
						timer.Create("PikPrimaryIdleAnim" .. ply:UniqueID(), .75, 1, function()
							ply:GetActiveWeapon():SendWeaponAnim(ACT_VM_IDLE)
						end)
...
function SWEP:SecondaryAttack()
	...
	if (IsValid(tr.Entity)) then
		for k, v in pairs(ents.FindByClass("pikmin")) do
			if (v.Olimar == self.Owner and (tr.Entity:IsNPC() or tr.Entity:IsPlayer())) then
				timer.Destroy("Pik2ndryIdleAnim" .. self.Owner:UniqueID())
				...
				timer.Create("Pik2ndryIdleAnim" .. self.Owner:UniqueID(), 1.3, 1, function()
					self:SendWeaponAnim(ACT_VM_IDLE)
				end)

				self.WhistleSound:Play()

				timer.Create("Pik2ndWhistle" .. self.Owner:UniqueID(), 1.45, 1, function()
